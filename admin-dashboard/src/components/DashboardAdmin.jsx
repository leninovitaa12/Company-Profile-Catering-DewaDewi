...
const formatTimeAgo = (timestamp) => {
  ...
  if (diffInMinutes < 1) return `${diffInSeconds} detik yang lalu`;
...
const DashboardAdmin = () => {
  ...
  const {
    testimonis,
    loadings: testimonisLoading,
    refetch: refetchTestimonis,
  } = useGetTestimoni();
  ...
  const activities = [
    ...
    ...logs.map((log) => ({
      ...
      icon: log.action.toLowerCase().includes("produk") ? (
        <ProductIcon />
      ) : log.action.toLowerCase().includes("testimoni") ? (
        <TestimoniIcon />
      ) : (
        <i className="fa-solid fa-clock" />
      ),
      color: log.action.toLowerCase().includes("produk")
        ? "text-blue-600"
        : log.action.toLowerCase().includes("testimoni")
        ? "text-green-600"
        : "text-purple-600",
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    >
                      <div className="bg-white/60 backdrop-blur-sm p-3 rounded-lg border border-gray-100">
                        <h4 className="font-medium text-[#D36B00] mb-1">
                          Produk Terbaru
                        </h4>
                        <p className="text-sm text-gray-600 truncate">
                          {latestProduct.name || "Belum ada produk"}
                        </p>
                      </div>
              ...
              <motion.div variants={itemVariants}>
                <ContentCard title="Aktivitas Terbaru">
                  <div className="space-y-4 max-h-[250px] overflow-y-auto scroll-thin overflow-x-hidden scrollbar-hide">
                    <AnimatePresence>
                      {activities.map((activity, index) => (
                        <motion.div
                          key={index}
                          className="flex cursor-pointer items-start gap-3 p-3 bg-white rounded-lg border border-gray-100 shadow-sm min-h-[80px]"
